LANGUAGE_NAME := tree-sitter-penny-dialog
HOMEPAGE_URL := https://github.com/snotbane/tree-sitter-penny-dialog.git
VERSION := 0.1.0
DESCRIPTION := Injected sub-grammar for Penny strings
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/penny-dialog '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/penny-dialog
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/penny-dialog
